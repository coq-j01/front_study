...
// 1.useState 훅
// 컴퍼넌트 내에서 상태를 관리할 수 있도록 도와주는 혹이다.
...
export function Counter5() {
  ...
  // useCallback으로 숫자 입력 핸들러(콜백함수) 메이이제이션
...
export function Counter6() {
  ...
  return (
    <div>
      <h2>Counter : {count}</h2>
      <h2>버튼클릭횟수: {clickCountRef.current}</h2>
...
export function Counter7() {
  ...
  const handleClick = () => {
    ...
    if (inputRef.current) {
      //null체크
      inputRef.current.value = `현재카운트: ${count}`;
